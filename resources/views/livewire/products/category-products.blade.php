<div class="row px-xl-5">
    <div class="col-lg-3 col-md-12">
        ...
        @forelse($brands AS $brand)
            <div class="custom-control custom-checkbox d-flex align-items-center justify-content-between mb-3">
                ...
                <span class="badge border font-weight-bold p-2">
                    {{ $brand->products()->whereHas('categories', function ($query) use ($category) {
        ...
        @empty
            <div class="alert alert-warning">No Brands for this cateogry</div>
        @endforelse
        {{-- ===END Brands --}}

        @forelse($attributes as $attribute)
            <div class="border-bottom mb-4 pb-4">
                <h5 class="font-weight-semi-bold mb-4">{{ $attribute->name }}
                    ({{ $attribute->products()->whereHas('categories', function ($qry) use ($category) {
                            $qry->where('id', $category->id);
                        })->count() }})
                </h5>
                @forelse($attribute->values as $value)
                    <div class="custom-control custom-checkbox d-flex align-items-center justify-content-between mb-3">
                        ...
                        <span class="badge border font-weight-bold p-2">
                            {{ $attribute->products()->wherePivot('value', $value->name)->whereHas('categories', function ($qry) use ($category) {
                                    $qry->where('id', $category->id);
                                })->count() }}
                        </span>
                ...
                <div class="col-lg-4 col-md-6 col-sm-12 pb-1 my-3">
                    ...
                    {{-- <div class="card product-item border-0 h-100 d-flex flex-column justify-content-between">
                        <div
                            ...
                            <span class="badge bg-info text-light float-right">{{ $product->brand->name }}</span>
                            <img class="img-fluid w-100" src="{{ $product->photoUrl() }}" alt="">
                        </div>
                        <div>
                            <div class="card-body border-left border-right text-center p-0 pt-4 pb-3">
                                <h6 class="text-truncate mb-3 p-1 border-top">{{ $product->name }}</h6>
