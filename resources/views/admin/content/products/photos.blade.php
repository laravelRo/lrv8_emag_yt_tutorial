...
@section('content')
    ...
    <main>
        <div class="container-fluid px-4">
            <h1 class="mt-4">Photo Galery for <span class="text-info">{{ $product->name }}</span></a>
            </h1>
            <ol class="breadcrumb mb-4">
                <li class="breadcrumb-item"><a href="{{ route('control-panel') }}">Control Panel</a></li>
                <li class="breadcrumb-item"><a href="{{ route('products.list', ['page' => $currentPage]) }}">Products</a>
                </li>
                <li class="breadcrumb-item active">{{ $product->name }} Photo Galery</li>
                <li class="breadcrumb-item"><a
                        href="{{ route('products.edit', ['id' => $product->id, 'currentPage' => $currentPage]) }}">Edit
                        Products <span class="text-info">{{ $product->name }}</span> - <span
                            class="text-danger">id:{{ $product->id }}</span></a>
                </li>
            </ol>

            <div class="row">
                @livewire('admin.photos-upload', ['model'=>$product, 'path'=>'products/'.$product->id.'/'])
            </div>
